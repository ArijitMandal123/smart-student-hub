  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const groupsRes = await api.get(`/api/teacher/groups/${teacherData.teacherId}`);
          ...
          for (const studentId of group.students) {
            ...
            if (student) {
              ...
              try {
                ...
                console.log(`Student ${studentId} marks:`, completeStudent.semesterMarks?.length || 0, 'CGPA:', completeStudent.cgpa);
                teacherStudents.push({ ...completeStudent, groupName: group.name });
              } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50">
      ...
      <div className="container mx-auto p-6">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <p className="text-gray-600 text-lg">Teacher ID: {teacherData?.teacherId}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl p-6 border border-white/20">
            ...
            <h3 className="text-lg font-semibold mb-2 text-gray-800">My Groups</h3>
            <p className="text-gray-600">Assigned student groups</p>
            <p className="text-2xl font-bold text-green-600 mt-2">{groups.length}</p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl p-6 border border-white/20">
            ...
            <h3 className="text-lg font-semibold mb-2 text-gray-800">Total Students</h3>
            <p className="text-gray-600">Students under supervision</p>
            <p className="text-2xl font-bold text-green-600 mt-2">{students.length}</p>
          </div>
        ...
        <div className="mt-8">
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl p-8 border border-white/20">
            <div className="flex gap-4 mb-6">
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                  activeTab === 'overview' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                }`}
              >
                Overview
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                  activeTab === 'groups' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                }`}
              >
                My Groups
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                  activeTab === 'students' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                }`}
              >
                Student Management
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                  activeTab === 'marks' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                }`}
              >
                Student Marks
              </button>
            ...
            {activeTab === 'overview' && (
              <div>
                <h3 className="text-xl font-bold mb-4">Teacher Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-gray-700">Name</label><p className="text-lg">{teacherData?.name}</p></div>
                  <div><label className="block text-sm font-medium text-gray-700">Teacher ID</label><p className="text-lg">{teacherData?.teacherId}</p></div>
                </div>
              </div>
            )}
                  ...
                  <div className="space-y-4">
                    {groups.map((group) => (
                      <div key={group._id} className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p className="text-gray-600">College: {group.college}</p>
                        <p className="text-gray-600">Department: {group.department}</p>
                        <p className="text-gray-600">Students: {group.students.length}</p>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    {students.length === 0 ? (
                      ...
                      <div className="grid gap-6">
                        {students.map((student) => (
                          <div key={student.studentId} className="bg-gray-50 p-6 rounded-lg border">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <p className="text-gray-900">{student.personalCertificates?.length || 0} certificates</p>
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Projects</label>
                                <p className="text-gray-900">{student.projects?.length || 0} projects</p>
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">CGPA</label>
                                <p className="text-gray-900 font-semibold text-green-600">{student.cgpa || 'N/A'}</p>
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Semester Records</label>
                                <p className="text-gray-900">{student.semesterMarks?.length || 0} semesters</p>
                              </div>
                            </div>
                  ...
                  <div>
                    <div className="flex items-center mb-4">
                      ...
                      <h3 className="text-xl font-bold">
                        {viewMode === 'profile' ? 'Student Profile' : 
                         viewMode === 'certificates' ? 'Student Certificates' : 
                         viewMode === 'projects' ? 'Student Projects' : 'Student Marks'}
                      </h3>
                    </div>
                      ...
                      <div className="bg-gradient-to-br from-white to-green-50 p-8 rounded-2xl shadow-xl border border-green-100">
                        <div className="flex items-center mb-6">
                          ...
                          <div>
                            ...
                            <p className="text-green-600 font-medium">{selectedStudent.groupName}</p>
                          </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                            ...
                            <p className="text-lg font-semibold text-gray-800">{selectedStudent.email}</p>
                          </div>
                          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                            <div className="flex items-center mb-2">
                            ...
                            <p className="text-lg font-semibold text-gray-800">{selectedStudent.department}</p>
                          </div>
                          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                            <div className="flex items-center mb-2">
                            ...
                            <p className="text-lg font-semibold text-gray-800">{selectedStudent.college}</p>
                          </div>
                          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                            <div className="flex items-center mb-2">
                            ...
                            <p className="text-lg font-semibold text-gray-800">Year {selectedStudent.year || 'N/A'}, Sem {selectedStudent.semester || 'N/A'}</p>
                          </div>
                          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                            <div className="flex items-center mb-2">
                            ...
                            <p className="text-lg font-semibold text-gray-800">{selectedStudent.rollNumber || 'N/A'}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {viewMode === 'certificates' && selectedStudent && (
                      <div className="bg-gradient-to-br from-white to-blue-50 p-8 rounded-2xl shadow-xl border border-blue-100">
                        <div className="flex items-center mb-6">
                          ...
                          <div>
                            <h4 className="text-2xl font-bold text-gray-800">{selectedStudent.name}'s Certificates</h4>
                            <p className="text-blue-600 font-medium">{selectedStudent.personalCertificates?.length || 0} certificates earned</p>
                          </div>
                        </div>
                        {selectedStudent.personalCertificates?.length > 0 ? (
                          <div className="grid gap-6">
                            {selectedStudent.personalCertificates.map((cert, index) => (
                              <div key={index} className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
                                <div className="flex gap-6">
                                  ...
                                  <div className="flex-1">
                                    <h5 className="text-xl font-bold text-gray-800 mb-3">{cert.name}</h5>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      ...
                                      <div className="flex items-center">
                                        ...
                                        <div>
                                          ...
                                          <span className="inline-block bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm font-medium">{cert.category}</span>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                          ...
                          <div className="text-center py-12">
                            ...
                            <p className="text-xl font-semibold text-gray-600 mb-2">No certificates found</p>
                            <p className="text-gray-500">This student hasn't uploaded any certificates yet.</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {viewMode === 'projects' && selectedStudent && (
                      <div className="bg-gradient-to-br from-white to-purple-50 p-8 rounded-2xl shadow-xl border border-purple-100">
                        <div className="flex items-center mb-6">
                          ...
                          <div>
                            <h4 className="text-2xl font-bold text-gray-800">{selectedStudent.name}'s Projects</h4>
                            <p className="text-purple-600 font-medium">{selectedStudent.projects?.length || 0} projects completed</p>
                          </div>
                        </div>
                        {selectedStudent.projects?.length > 0 ? (
                          <div className="grid gap-6">
                            {selectedStudent.projects.map((project, index) => (
                              <div key={index} className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
                                <div className="flex justify-between items-start mb-4">
                                  <div className="flex-1">
                                    <h5 className="text-xl font-bold text-gray-800 mb-2">{project.title}</h5>
                          ...
                          <div className="text-center py-12">
                            ...
                            <p className="text-xl font-semibold text-gray-600 mb-2">No projects found</p>
                            <p className="text-gray-500">This student hasn't added any projects yet.</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {viewMode === 'marks' && selectedStudent && (
                      <div className="bg-gradient-to-br from-white to-orange-50 p-8 rounded-2xl shadow-xl border border-orange-100">
                        <div className="flex items-center mb-6">
                          ...
                          <div>
                            <h4 className="text-2xl font-bold text-gray-800">{selectedStudent.name}'s Academic Records</h4>
                            <p className="text-orange-600 font-medium">CGPA: {selectedStudent.cgpa || 'N/A'}</p>
                          </div>
                        </div>
                        {selectedStudent.semesterMarks?.length > 0 ? (
                          <div className="grid gap-6">
                            {selectedStudent.semesterMarks.map((record, index) => (
                              <div key={index} className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                                <div className="flex justify-between items-center mb-4">
                                  <h5 className="text-xl font-bold text-gray-800">Semester {record.semester} - Year {record.year}</h5>
                                ...
                                {record.subjects?.length > 0 && (
                                  <div className="grid gap-3">
                                    <h6 className="font-semibold text-gray-700 mb-2">Subject Details:</h6>
                                    {record.subjects.map((subject, subIndex) => (
                                      <div key={subIndex} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                                        <span className="font-medium text-gray-800">{subject.name}</span>
                                        <div className="flex gap-4">
                                          <span className="text-gray-600">Marks: {subject.marks}</span>
                                          <span className="font-semibold text-blue-600">Grade: {subject.grade}</span>
                                        </div>
                                      </div>
                                    ))}
                                  </div>
                          ...
                          <div className="text-center py-12">
                            ...
                            <p className="text-gray-500">This student's semester marks haven't been added yet.</p>
                          </div>
                        )}
                      </div>
                    )}
            ...
            {activeTab === 'marks' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-bold">Student Marks Management</h3>
                  <button
                    onClick={() => {
                      setBulkMarksData({
                        semester: '',
                        ...
                        <div key={student.studentId} className="p-4 hover:bg-gray-50 transition-colors duration-200">
                          <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                              <p className="text-sm text-gray-600">CGPA: <span className="font-semibold text-green-600">{student.cgpa || 'N/A'}</span></p>
                              <p className="text-sm text-gray-600">Semesters: {student.semesterMarks?.length || 0}</p>
                            </div>
                          </div>
                        </div>
                      ))}
      ...
      {showImagePopup && selectedImage && (
        <div className="fixed inset-0 backdrop-blur-lg bg-gradient-to-br from-blue-100/30 via-purple-100/20 to-pink-100/30 flex items-center justify-center z-50 animate-fadeIn" onClick={() => setShowImagePopup(false)}>
          <div className="bg-white/90 backdrop-blur-xl border border-white/20 shadow-2xl rounded-2xl max-w-6xl w-full mx-4 p-8 animate-slideUp" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Certificate Image</h2>
              <button 
          ...
          <div className="bg-white/95 backdrop-blur-xl border border-white/20 shadow-2xl rounded-2xl max-w-6xl w-full mx-4 my-8 p-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                Bulk Marks Entry - Semester {bulkMarksData.semester} Year {bulkMarksData.year}
              </h2>
              <button 
            ...
            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                ...
                onChange={(e) => setBulkMarksData({...bulkMarksData, semester: e.target.value})}
              ...
              <input
                ...
                onChange={(e) => setBulkMarksData({...bulkMarksData, year: e.target.value})}
                className="p-3 border rounded-lg font-medium"
                required
              />
            </div>
            ...
            <div className="max-h-96 overflow-y-auto border border-gray-200 rounded-b-xl">
              {bulkMarksData.students.map((student, studentIndex) => (
                <div key={student.studentId} className="border-b border-gray-200 p-4 bg-white hover:bg-gray-50">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setBulkMarksData({...bulkMarksData, students: newStudents});
                      }}
                  ...
                  <div className="space-y-2">
                    {student.subjects.map((subject, subjectIndex) => (
                      <div key={subjectIndex} className="grid grid-cols-1 md:grid-cols-5 gap-2">
                        <input
                          type="text"
                          ...
                          onChange={(e) => {
                            const newStudents = [...bulkMarksData.students];
                            newStudents[studentIndex].subjects[subjectIndex].name = e.target.value;
                            setBulkMarksData({...bulkMarksData, students: newStudents});
                          }}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newStudents = [...bulkMarksData.students];
                            newStudents[studentIndex].subjects[subjectIndex].marks = e.target.value;
                            setBulkMarksData({...bulkMarksData, students: newStudents});
                          }}
                          className="p-2 border rounded text-sm"
                        />
                        <input
                          ...
                          onChange={(e) => {
                            const newStudents = [...bulkMarksData.students];
                            newStudents[studentIndex].subjects[subjectIndex].grade = e.target.value;
                            setBulkMarksData({...bulkMarksData, students: newStudents});
                          }}
                          className="p-2 border rounded text-sm"
                        />
                        <button
                          ...
                          onClick={() => {
                            const newStudents = [...bulkMarksData.students];
                            newStudents[studentIndex].subjects = newStudents[studentIndex].subjects.filter((_, i) => i !== subjectIndex);
                            setBulkMarksData({...bulkMarksData, students: newStudents});
                          }}
                        ...
                        <button
                          ...
                          onClick={() => {
                            const newStudents = [...bulkMarksData.students];
                            newStudents[studentIndex].subjects.push({ name: '', marks: '', grade: '' });
                            setBulkMarksData({...bulkMarksData, students: newStudents});
                          }}
                ...
                onClick={async () => {
                  try {
                    ...
                    for (const student of bulkMarksData.students) {
                      console.log('Processing student:', student.name, 'SGPA:', student.sgpa);
                      
                      if (student.sgpa && parseFloat(student.sgpa) > 0) {
                        const validSubjects = student.subjects.filter(s => s.name && s.marks && s.grade);
                        ...
                        console.log('Saving marks for student:', student.studentId, marksData);
                        
                        try {
                          const response = await api.post(`/api/students/${student.studentId}/marks`, marksData);
                          console.log('Success response:', response.data);
                          savedCount++;
                        } catch (error) {
                          console.error('Error saving for student:', student.name, error.response?.data || error.message);
                          errors.push(`${student.name}: ${error.response?.data?.error || error.message}`);
                        }
                      } else {
                        console.log('Skipping student (no SGPA):', student.name);
                      }
                    }
                    let message = `Marks saved successfully for ${savedCount} students!`;
                    if (errors.length > 0) {
                      message += `\n\nErrors:\n${errors.join('\n')}`;
                    }
                    ...
                    if (savedCount > 0) {
                      ...
                      setBulkMarksData({ semester: '', year: '', students: [] });
